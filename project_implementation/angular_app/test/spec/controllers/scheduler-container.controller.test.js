...
describe('Controller: SchedulerContainer', function () {
  var SchedulerContainer,
      scope,
      sandbox, schedulerService;
  ...
  beforeEach(inject(function ($rootScope, $controller) {
    ...
    SchedulerContainer = $controller('SchedulerContainer', {
      $scope: scope
    });
  }));
  ...
  describe('submitForm', function () {
    ...
    it('should not login with invalid username', function () {
      sandbox.stub(schedulerService, 'loginUser');
      ...
      expect(schedulerService.loginUser.calledOnce).toBe(false);
    });
    ...
    it('should login with valid credentials', inject(function($q) {
      sandbox.stub(schedulerService, 'loginUser', function(){
      ...
      expect(schedulerService.loginUser.calledOnce).toBe(true);
    }));
    ...
    it('should register with valid credentials', inject(function($q) {
      sandbox.stub(schedulerService, 'registerUser', function(){
        var deferred = $q.defer();
        deferred.resolve();
        return deferred.promise;
      });
      ...
      expect(schedulerService.registerUser.calledOnce).toBe(true);
    }));
  });
